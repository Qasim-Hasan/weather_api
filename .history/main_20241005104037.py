...
async def create_admin_route(admin: AdminCreate, db: Session = Depends(db_dependency), background_tasks: BackgroundTasks = Depends()):
    result = create_admin(db, admin, background_tasks)  # Ensure background task is passed
