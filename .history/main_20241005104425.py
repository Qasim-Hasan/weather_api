...
async def create_admin_route(admin: AdminCreate, db: db_dependency, background_tasks: BackgroundTasks = Depends()):
    result = create_admin(db, admin, background_tasks)  # Pass the background task to send email
