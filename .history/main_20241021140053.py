...
from data_generation.get_data import run_data_generation
import time
...
def periodic_data_generation():
    while True:
        run_data_generation()  # Your function to generate data
        time.sleep(60)  # Run every 1 minute


# Trigger background tasks when the application starts
@app.on_event("startup")
async def startup_event():
    # Start the background task to run periodically
    app.add_task(periodic_data_generation)